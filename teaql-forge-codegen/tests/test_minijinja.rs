...
fn test_data_design_react() {
    ...
    env.add_template("data-design-react", &template_content).unwrap();
    let result = env.get_template("data-design-react").unwrap().render(minijinja::context! { domain => render_domain }).unwrap();
