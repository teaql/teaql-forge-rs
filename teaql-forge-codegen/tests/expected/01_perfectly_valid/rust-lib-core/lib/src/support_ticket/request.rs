...
impl<R> SupportTicketRequest<R> {
    pub(crate) fn new() -> Self {
        Self {
            query: SelectQuery::new("SupportTicket")
                .project("id")
                .project("version"),
        ...
        match head {
            "status" => {
                self.with_status_matching(
                    crate::Q::ticket_statuses_minimal()
                        .apply_dynamic_json_filter(tail, value),
                )
            }
            "customer_issue_list" => {
                self.with_customer_issue_list_matching(
                    crate::Q::customer_issues_minimal()
                        .apply_dynamic_json_filter(tail, value),
                )
            }
            _ => self,
    ...
    /// let dynamic_query = crate::Q::ticket_statuses_minimal().filter(...);
    /// let request = crate::Q::support_tickets().with_status_matching(dynamic_query);
    /// ```
    pub fn with_status_matching(mut self, request: impl Into<QuerySelection>) -> Self {
        let selection = request.into();
        self.query = self.query.and_filter(Expr::in_subquery(
            "status_id",
            <crate::TicketStatus as teaql_core::TeaqlEntity>::entity_descriptor(),
    ...
    /// // Only use when building dynamic queries
    /// let dynamic_query = crate::Q::ticket_statuses_minimal().filter(...);
    /// let request = crate::Q::support_tickets().without_status_matching(dynamic_query);
    /// ```
    ...
    pub fn with_customer_issue_list_matching(mut self, request: impl Into<QuerySelection>) -> Self {
        ...
        self.query = self.query.and_filter(Expr::in_subquery(
            "id",
            <crate::CustomerIssue as teaql_core::TeaqlEntity>::entity_descriptor(),
            selection.query.clone(),
            "ticket_id",
        ));
    ...
    pub fn select_customer_issue_list_with(mut self, request: impl Into<QuerySelection>) -> Self {
        ...
        self.relation_selections.push(RelationSelection::new("customer_issue_list", selection));
        self
}
