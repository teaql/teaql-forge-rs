...
impl E {
    pub fn ticket_status<'a>(value: &'a crate::TicketStatus) -> crate::TicketStatusExpression<'a> {
        let root_desc = std::sync::Arc::new(format!("TicketStatus(id={})", value.id()));
        crate::TicketStatusExpression::new(teaql_core::eval::EvalResult::Value(value), root_desc)
    }
    ...
    pub fn customer_issue<'a>(value: &'a crate::CustomerIssue) -> crate::CustomerIssueExpression<'a> {
        let root_desc = std::sync::Arc::new(format!("CustomerIssue(id={})", value.id()));
        crate::CustomerIssueExpression::new(teaql_core::eval::EvalResult::Value(value), root_desc)
    }
}
